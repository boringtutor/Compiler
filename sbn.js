...
function parser(tokens) {
  ...
  function createDot(current_token, currentPosition, node) {
    ...
    if (currentPosition < expectedLength - 1) {
      if (
        expectedTypeCheck(current_token.type, expectedType[currentPosition])
      ) {
  ...
  function findArguments(
    ...
    while (expectedLength > currentPosition) {
      ...
      if (expectedType) {
        var expected = expectedTypeCheck(
          token.type,
          expectedType[currentPosition]
        );
        if (!expected) {
          throw (
            command +
            " takes " +
            JSON.stringify(expectedType[currentPosition]) +
            " as argument " +
            (currentPosition + 1) +
            ". " +
            (token
...
function generator(ast) {
  function traverseSvgAst(obj, parent, rest, text) {
    ...
    while (obj.length > 0) {
      ...
      var attr = Object.keys(currentNode.attr)
        .map(function (key) {
